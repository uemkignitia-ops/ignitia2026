      ...
      mode === "development" && componentTagger(),
      {
        ...
        configureServer(server: ViteDevServer) {
          server.middlewares.use((req: IncomingMessage, res: ServerResponse, next: () => void) => {
            console.log("[DEBUG SERVER] URL:", req.url);
            if (req.url?.startsWith("/api/upload") || req.url?.startsWith("/api/delete") || req.url?.startsWith("/api/verify-seed")) {
              let body = "";
              ...
              req.on("end", async () => {
                try {
                  ...
                  } else if (req.url?.startsWith("/api/verify-seed")) {
